...
function renderResults(){
  ...
  els.cards.innerHTML=state.results.map(({file,type,report:r})=>{
    ...
    const rows=r.issues.slice(0,500).map(x=>`<tr><td>${x.row}</td><td>${escapeHtml(x.kind)}</td><td>${escapeHtml(x.detail)}</td></tr>`).join("");
    return `<article class="result-card ${css}"><div class="result-head"><div><h3>${escapeHtml(file)}</h3><small>${escapeHtml(type.toUpperCase())} · validación independiente</small></div><span class="status">${r.status}</span></div><div class="result-stats">${stat(r.total,"Registros")}${stat(r.matched,"Coincidencias")}${stat(r.missing,"Faltantes")}${stat(r.differences,"Diferencias")}${stat(r.errors,"Total alertas")}</div>${r.issues.length?`<details><summary>Ver detalle (${r.issues.length})</summary><div class="issues"><table><thead><tr><th>Fila</th><th>Tipo</th><th>Detalle</th></tr></thead><tbody>${rows}</tbody></table></div></details>`:"<p>Todos los registros del archivo coinciden con el TXT base.</p>"}</article>`;
  }).join("");
