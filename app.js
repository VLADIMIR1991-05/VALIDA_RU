...
els.base.addEventListener("change",async e=>{const f=e.target.files[0];if(!f)return;state.base=f;state.baseLines=ValidaRU.parseBase(await f.text());$("#base-zone").classList.add("loaded");$("#base-title").textContent=f.name;$("#base-meta").textContent=`${humanSize(f.size)} · ${state.baseLines.length.toLocaleString("es-EC")} líneas`;refresh()});
...
function renderResults(){
  ...
  els.cards.innerHTML=state.results.map(({file,type,report:r})=>{
    ...
    const important=(r.issues.length?r.issues:r.comparisons).slice(0,50);
    const rows=important.map(x=>`<tr><td>${x.row}</td><td>${escapeHtml(x.kind)}</td><td>${escapeHtml(x.reference||"—")}</td><td>${escapeHtml(x.xls||x.detail||"—")}</td><td class="txt-cell">${escapeHtml(x.txt||"—")}</td></tr>`).join("");
    ...
    return `<article class="result-card ${css}"><div class="result-head"><div><h3>${escapeHtml(file)}</h3><small>${escapeHtml(type.toUpperCase())} · validación independiente</small></div><span class="status">${r.status}</span></div><div class="result-stats">${stat(r.total,"Registros")}${stat(r.matched,"Coincidencias directas")}${stat(r.missing,"Faltantes importantes")}${stat(r.differences,"Diferencias importantes")}${stat(r.errors,"Total alertas")}</div>${note}<details><summary>${label}</summary><p class="detail-help">${r.issues.length?"Se muestran primero los errores que requieren revisión.":"Muestra representativa; no se listan todos los registros correctos."}</p><div class="issues"><table><thead><tr><th>Fila XLS</th><th>Resultado</th><th>Código/pieza</th><th>Valor XLS</th><th>Línea TXT relacionada</th></tr></thead><tbody>${rows}</tbody></table></div></details></article>`;
  }).join("");
